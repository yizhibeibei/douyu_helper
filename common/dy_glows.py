...
def glow_donate(num=1, room_id=12306):
    ...
    if Bags:
        ...
        try:
            ...
            Own = now_left
            logger.info("房间号 %s 赠送荧光棒 %s个" % (room_id, num))
        except AssertionError:
...
def go_room():
    ...
    if "UserInfo" in a.get_attribute("class"):
        logger.debug("成功以登陆状态进入页面")
        logger.debug("如提示背包没有荧光棒请延长等待时间")
    else:
        logger.error("没有携带cookie进入页面,请重新检查cookie")
    logger.debug("再次刷新页面")
    driver.refresh()
    sleep(10)
    driver.quit()
